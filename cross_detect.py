...
def load_data_gens(gen_a_file, gen_b_file, original_b=False,original_a=False):

    data = {'original':[],'sampled':[]}

    if original_b and not original_a:
        for  line_b in open(gen_b_file,'r'):
            di_b= json.loads(line_b)

            #we only want the sampled stuff - gen_a is original and gen_b is sampled

            for element in di_b['original']:
                data['original'].append(element)
            
            for element in di_b['sampled']:
                data['sampled'].append(element)

    elif original_b and  original_a:

        for line_a , line_b in zip(open(gen_a_file,'r'),open(gen_b_file,'r')):
            di_a= json.loads(line_a)
            di_b= json.loads(line_b)

            #we only want the sampled stuff - gen_a is original and gen_b is sampled

            for element in di_a['original']:
                    data['original'].append(element)
            
            for element in di_b['original']:
                data['sampled'].append(element)

    elif original_a and not original_b:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_a', type=str, default="")
    parser.add_argument('--gen_b', type=str, default="")
    parser.add_argument('--original_b', type=bool, default=False)
    parser.add_argument('--original_a', type=bool, default=False)
    ...
    if args.original_a and args.original_b:
        suf_org = 'org_org'
    elif args.original_a and not args.original_b:
        suf_org="orga"
    elif args.original_b and not args.original_a:
        suf_org="" #TODO OOOO
    else:
        suf_org = "gen_gen"
    
    n_perturbation_list = [int(x) for x in args.n_perturbation_list.split(",")]
    if len(n_perturbation_list) == 1  and n_perturbation_list[0] == 25:
        suf_pert = ""
    else:
        suf_pert = f"_{n_perturbation_list[0]}"

    SAVE_FOLDER = f"tmp_results_cross/{output_subfolder}{base_model_name}-{args.revision}{scoring_model_string}-{args.gen_a.replace('/','_')}-{args.gen_b.replace('/','_')}{suf_org}{suf_pert}"
    ...
    gen_a_file = f'{args.root_gen}/{args.gen_a}/raw_data.json'
    gen_b_file = f'{args.root_gen}/{args.gen_b}/raw_data.json'
    ...
    data = load_data_gens(gen_a_file,gen_b_file,args.original_b,args.original_a) #we will return gen_a as original (human) and gen_b as the generated text
